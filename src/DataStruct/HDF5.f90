!-------------------------------------------------------------------------------
! (c) Copyright, 2020 by the Regents of the University of California.
! HDF5class: Class to read and write HDF5 files in parallel.
!
! MODULE  : ... HDF5class
! VERSION : ... 0.1
!> @author
!> Matt Easton, Peking University
!
! DESCRIPTION:
!> This class defines types and functions to save and write to HDF5 files in
!> OpenPMD-compliant format, which are used to output particle and field data
!> for post-processing and visualisation.
! Comments:
!> Ported from the Fortran OpenPMD File Writer project:
!> https://github.com/UCLA-Plasma-Simulation-Group/Fortran-OpenPMD-File-Writers
!> This module is based on their `hdf5io_class` module, downloaded 2020-05-28
!> (commit ede4aa44032c15ee37ad773ee0b27ff80c74cb82)
!-------------------------------------------------------------------------------
...
module HDF5class
    ...
    interface pwpart
        ...
        module procedure pwpart_array_double
        module procedure pwpart_const
        module procedure pwpart_const_double
    end interface

contains

    subroutine init_hdf5file(this, openPMD, openPMDextension, iter, base,   &
                            &basePath, meshesPath, particlesPath,           &
                            &iterationEncoding, iterationFormat,            &
                            &filenamebase, time, dt, timeUnitSI, records,   &
                            &component, geometry, geometryParameters,       &
                            &dataOrder, axisLabels, gridSpacing,            &
                            &gridGlobalOffset, gridUnitSI, position,        &
                            &particleName, unitSI, unitDimension,           &
                            &timeOffset, filepath)
        ...
        if (present(gridGlobalOffset)) then
            if(associated(this%gridGlobalOffset)) then
                deallocate(this%gridGlobalOffset)
            end if
    ...
    subroutine createfile(file,ierr)
        ...
        call add_h5_atribute(rootID, 'iterationEncoding', &
                            &trim(file%iterationEncoding))
        call add_h5_atribute(rootID, 'iterationFormat', &
                            &trim(file%iterationFormat))
    ...
    subroutine pwfield_3d(file,fd,gs,ls,noff,ierr)
        ...
        call h5dcreate_f(meshid, trim(file%records), treal, dspace_id, &
                        &dset_id, ierr, dcplID)
        ...
        call add_h5_atribute(dset_id, 'geometryParameters', &
                            &trim(file%geometryParameters))
        ...
        if (associated(file%gridGlobalOffset)) then
            call add_h5_atribute(dset_id, 'gridGlobalOffset', &
                                &file%gridGlobalOffset)
        else
    ...
    subroutine prfield_3d(file,fd,gs,ls,noff,ierr)
    ...
    subroutine pwpart_array(file,part,npp,ierr)
        ...
        if (tnpp > 0) then
            ...
            call MPI_ALLGATHER(tnpp, 1, MPI_INTEGER, np, 1, MPI_INTEGER, pgrp, &
                              &ierr)
            dims(1, 1) = 1
            dims(2, 1) = np(1)
            do i = 2, pnvp
            ...
            call h5fopen_f(trim(file%filename), H5F_ACC_RDWR_F, file_id, ierr, &
                          &access_prp=flplID)
            call h5gopen_f(file_id, '/', rootID, ierr)
            call h5gopen_f(rootID, trim(file%base)//trim(file%chiter), iterid, &
                          &ierr)
            ...
            call h5dcreate_f(recid, trim(file%component), treal, dspace_id, &
                            &dset_id, ierr)
            ldim(1) = tnpp
    ...
    subroutine pwpart_array_double(file, part, npp, ierr)
        ...
        call pwpart_array(file, real(part(:)), npp, ierr)

    end subroutine pwpart_array_double
    ...
    subroutine pwpart_const_double(file, value, ierr)
        ...
        call pwpart_const(file, real(value), ierr)

    end subroutine pwpart_const_double
