    ...
    subroutine run_AccSimulator()
        ...
        do i = iend+1, ntstep
            ...
            if(associated(Blnelem(ibend)%prfq)) then
                ...
                rmin=2*0.01*rtest
            else if (associated(Blnelem(ibend)%pdtl)) then
                ! call getfldt_DTL(post,momentumt,fldtest,Blnelem(icell)%pdtl,fldmp(icell))
            else if (associated(Blnelem(ibend)%pfldmp)) then
                        ...
                        do ib = 1, ibunch
                            ...
                            if(dflagwake.eq.1) then
                                ...
                                if(zmin.gt.dtwkstart(1).and.zmax.lt.dtwkend(1).and.iwrite.eq.1) then
                                    do kz=1,Nzlocal
                                        write(73,*)(kz-1)*hz,dexwake(Nx,Ny,kz),deywake(Nx,Ny,kz),&
                                                    dezwake(Nx,Ny,kz),dbxwake(Nx,Ny,kz),&
                                                    dbywake(Nx,Ny,kz),dbzwake(Nx,Ny,kz)
                                        write(72,*)(kz-1)*hz,exg(Nx,Ny,kz),eyg(Nx,Ny,kz),&
                                                    ezg(Nx,Ny,kz),bxg(Nx,Ny,kz),&
